...
void vmr_dump(vmr_t *r, int i) {
    printk("memory %4d: [0x%08p : 0x%08p] %13ld [%7s] [%s%s%s%s] [%s-%s] refs: %ld|\n", i,
           ...
           __vmr_isstack(r) ? "stack" : __vmr_exec(r) ? ".text"
                                : __vmr_rw(r)     ? ".data"
                                : __vmr_read(r)   ? ".rodata"
                                                  : "inval",
...
int mmap_alloc(mmap_t **ref) {
    ...
    if ((mmap = kzalloc(sizeof *mmap)) == NULL) {
        pmman.free(pgdir);
        return -ENOMEM;
    }
...
int mmap_find_holeat(mmap_t *mmap, uintptr_t addr, size_t size, uintptr_t *paddr, int whence) {
    ...
    *paddr = 0;                     // Initialize the output address to 0
    ...
    } else if (whence == __whence_end) { // If searching from the end
...
int mmap_map_region(mmap_t *mmap, uintptr_t addr, size_t len, int prot, int flags, vmr_t **pvmr) {
    ...
    if (!mmap || (!__flags_fixed(flags) && !pvmr) || len == 0){
        return -EINVAL;
    }
...
int mmap_set_focus(mmap_t *mmap, uintptr_t *ref) {
    if (mmap == NULL) {
        return -EINVAL;
    }
    ...
    arch_switch_pgdir(mmap->pgdir, ref);
    return 0;
}
