...
extern int sched_wait(queue_t *wait_queue, tstate_t state, wakeup_t *preason, spinlock_t *lock);
extern int sched_wait_whence(queue_t *wait_queue, tstate_t state, queue_relloc_t whence, wakeup_t *preason, spinlock_t *lock);
...
extern int sched_wakeup(queue_t *wait_queue, wakeup_t reason);
extern int sched_wakeup_whence(queue_t *wait_queue, wakeup_t reason, queue_relloc_t whence);
...
#define wait_event_interruptible(wq, condition, lock) ({ \
    int __ret = 0;                                       \
    while (!(condition)) {                               \
        __ret = sched_wait(wq, T_SLEEP, lock);           \
        if (__ret) break;                                \
    }                                                    \
    __ret;                                               \
})
