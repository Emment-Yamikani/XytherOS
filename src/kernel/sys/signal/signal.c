...
int signal_init(signal_t *signals) {
    sigsetempty(&signals->sig_mask);
    
    for (usize signo = 0; signo < NSIG; ++signo) {
        int err = queue_init(&signals->sig_queue[signo]);
        if (err != 0) {
            kfree(signals);
            return err;
        }
    ...
    spinlock_init(&signals->sig_lock);
    return 0;
}

void signal_reset(signal_t *signals) {
    signal_assert_locked(signals);

    for (int sig = 0; sig < NSIG; ++sig) {
        sigqueue_flush(&signals->sig_queue[sig]);
    }

    // reset all signal actions.
    for (int signo = 0; signo < NSIG; ++signo) {
        signals->sig_action[signo].sa_flags     = 0;
        signals->sig_action[signo].sa_handler   = NULL;
        sigsetempty(&signals->sig_action[signo].sa_mask);
    }
...
void signal_free(signal_t *signals) {
    assert(signals, "Invalid signals.\n");

    spin_recursive_lock(&signals->sig_lock);

    for (usize signo = 0; signo < NSIG; ++signo) {
        queue_lock(&signals->sig_queue[signo]);
        queue_flush(&signals->sig_queue[signo]);
        queue_unlock(&signals->sig_queue[signo]);

        signals->sig_action[signo].sa_handler = SIG_DFL;
    }

    spin_unlock(&signals->sig_lock);
...
int siginfo_alloc(siginfo_t **psiginfo) {
    ...
    if (NULL == (siginfo = kzalloc(sizeof *siginfo))) {
        return -ENOMEM;
    }
...
void siginfo_dump(siginfo_t *siginfo) {
    ...
    printk(
        ...
        siginfo->si_addr,   siginfo->si_status,
        siginfo->si_value.sigval_ptr
    );
}

int signal_enqueue(signal_t *signals, siginfo_t *siginfo) {
    if (!signals || !siginfo || SIGBAD(siginfo->si_signo)) {
        return -EINVAL;
    }
    ...
    if ((err = sigqueue_enqueue(&signals->sig_queue[siginfo->si_signo - 1], siginfo))) {
        if (!queue_length(&signals->sig_queue[siginfo->si_signo - 1])) {
            sigsetdel(&signals->sigpending, siginfo->si_signo);
        }
    }
    queue_unlock(&signals->sig_queue[siginfo->si_signo - 1]);
    return err;
}
