...
static void x86_64_signal_onstack(void) {
    bool is_sigctx = current_is_handling_signal();

    current_set_handling_signal();
...
static int x86_64_dispatch_onstack(arch_thread_t *arch, sigaction_t *act, siginfo_t *siginfo) {
    u64         *kstack     = NULL;
