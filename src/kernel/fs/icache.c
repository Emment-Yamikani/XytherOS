...
update:
    if ((err = (size = iread_data(icache->pc_inode, pgno * PGSZ, buffer, PGSZ))) < 0) {
        goto error;
    }
    ...
    if ((err = arch_memcpyvp(paddr, (uintptr_t)buffer, size))) {
        goto error;
    }
...
ssize_t icache_read(icache_t *icache, off_t off, void *buffer, size_t sz) {
    ssize_t     err         = 0;
    uintptr_t   paddr       = 0;
    ...
    for (; sz; sz -= size, total += size, offset += size) {
        ...
        if ((err = (ssize_t)arch_memcpypv((uintptr_t)buffer + total,
...
ssize_t icache_write(icache_t *icache, off_t off, const void *buffer, size_t sz) {
    ssize_t     err     = 0;
    off_t       pgno    = 0;    /*page number*/
